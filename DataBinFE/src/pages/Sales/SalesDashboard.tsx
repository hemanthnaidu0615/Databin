...
export const SalesDashboard = () => {
  ...
  function separateOrderChannels(data: SalesData): SalesData[] {
    ...
    data.forEach((item) => {
      ...
      const now = moment();
      const format =
        now.diff(orderDate, "hours") < 24 ? "DD-MM HH:mm:ss" : "DD-MM";
          ...
          {salesDetails.map((item, index) => (
            <React.Fragment key={item.label}>
              <div className="flex flex-col items-center flex-grow mx-1 min-w-max">
                ...
                <div
                  className={`${item.color} h-1.5 mt-1 w-full max-w-[100px] rounded-b-lg`}
                ></div>
              </div>
              ...
              >
                ...
                <div className="card flex gap-2  m-2 h-full py-1 px-2  align-center justify-center">
                  <div className="card  border border-gray-200 shadow-lg rounded-lg  py-1 px-2 m-2 flex-1 w-1/2">
                    ...
                    <CustomDataTable
                      data={formatCustomDataTableData(
                        (type?.topItemsData?.byVolume || []).map((item) => ({
                          ...item,
                          web_category:
                            item.web_category === "Donation Skus"
                              ? "Donation"
                              : item.web_category,
                        })),
                        ["line_ordered_qty"]
                  ...
                  <div className="card border border-gray-200 shadow-lg rounded-lg  py-1 px-2 m-2 flex-1 w-1/2">
                    ...
                    <CustomDataTable
                      data={formatCustomDataTableData(
                        (type?.topItemsData?.byValue || []).map((item) => ({
                          ...item,
                          web_category:
                            item.web_category === "Donation Skus"
                              ? "Donation"
